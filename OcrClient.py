                    ...
                    if output_detect_img:
                        ...
                        if len(bb) > 0:
                            ...
                            cv2.putText(detect_img, '#%d' % block_one['id'], (
                                x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            if output_detect_img:
...
class OcrClient:   
    def search(img:str):
        ...
        if os.path.isfile(img):
            ...
            d.extend([img+'/'+a for a in os.listdir(img)])
        else:
            return ("Input file in not file or directory.")

        ocr_result = get_ocr(d, dpi=72, use_cuda=False, output_detect_img=False, low_gpu_memory=False)
